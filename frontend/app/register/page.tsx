...
export default function RegisterPage() {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center p-4 relative bg-[#0a0f1c] overflow-hidden">
      <div className="absolute top-1/4 right-1/4 w-[500px] h-[500px] bg-[#2563eb]/20 rounded-full blur-[120px] animate-pulse-glow pointer-events-none"></div>
      <div className="absolute bottom-1/4 left-1/4 w-[400px] h-[400px] bg-blue-900/20 rounded-full blur-[100px] animate-pulse-glow pointer-events-none" style={{ animationDelay: '1s' }}></div>
      ...
      <div className="glass-panel p-8 md:p-10 w-full max-w-md relative z-10 animate-slide-up shadow-[0_0_50px_rgba(37,99,235,0.1)] border-slate-700/50">
        ...
        <div className="text-center mb-8 mt-6">
          ...
          <p className="text-sm text-slate-400">Join your organization&apos;s AI workspace</p>
        </div>

        <form onSubmit={handleRegisterSubmit} className="space-y-4">
          <div>
            <label className="text-sm font-medium text-slate-300 ml-1 mb-1 block">Full Name</label>
            <div className="relative">
              <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500" />
              <input type="text" required placeholder="John Doe" className="w-full pl-10 pr-4 py-3 bg-slate-900/50 border border-slate-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#2563eb] text-slate-200 placeholder-slate-500 transition-all" />
            </div>
          </div>

          <div>
            <label className="text-sm font-medium text-slate-300 ml-1 mb-1 block">Work Email</label>
            <div className="relative">
          ...
          <button type="submit" className="btn-primary w-full mt-6 py-3 shadow-[0_0_15px_rgba(37,99,235,0.3)]">
            Create Account
          </button>
        </form>
        ...
        <p className="text-center mt-8 text-sm text-slate-400">
          Already have an account? <Link href="/login" className="text-[#2563eb] hover:text-[#1d4ed8] font-medium transition-colors">Sign in</Link>
        </p>
      </div>
    </main>
  );
